...
considering standards :) To make things worse, only very few are open source.
...
the type of IMAP server is what we're relly interested in, it brings together
the information from the upper two files.

Each IMAP server type ("vendor") is listed along with a list of IMAP
capabilities and all hosts which are know to belong to this specific type. The
list of capabilities is not necessarily identical for all IMAP servers of this
specific type though - you can think of it as a "fingerprint" for now.
...
be considered a really poor example of ERM. Anyway, it should be obvious now
that it would be a pain to maintain these files by hand. With that in mind, you
may wonder why it's YAML then. I wish I knew. One idea of mine is to put all of
this information into a single file, get rid of the pseudo-ids altogether, and
provide a tool to auto-generate that config asap.
